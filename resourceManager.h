...
/*
	Notes about resource manager:
	В момент вызова функции getAsset с передачей в нее умного указателя на ассет, по этому указателю либо размещается копия 
	(если такой объект ранее уже загружен), либо мгновенно создается пустой экземпляр, который затем некоторое время в фоне
	загружается. Во время загрузки он не используется.
	Следовательно, нужны дополнительные ухищрения, чтобы корректно вводить объекты в игру
